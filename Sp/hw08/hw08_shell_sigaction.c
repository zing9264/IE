...
void handler(int sig,siginfo_t *fdsi,void *ctx)
{
    if(sig==SIGINT){
        if (child_pid > 0) {
          int ret = kill(child_pid, fdsi->si_signo);
          if (ret == -1) {
      ...
      if(sig==SIGCHLD){//假設child結束
        ...
        if (WIFEXITED(wstatus)) { // child執行結束
          ...
          printf(RED "the child process was terminated by a signal " YELLOW
                 ...
                 WTERMSIG(fdsi->si_status),
                 sys_siglist[WTERMSIG(fdsi->si_status)]);
        }
        printf(NONE);
        child_pid = -1;
        printPrompt();
...
int main(int argc, char **argv) {
  ...
  sigfillset(&action.sa_mask); //監聽所有可以監聽的signal
  sigaction(SIGINT, &action,NULL);
  sigaction(SIGCHLD, &action,NULL);
  sigaction(SIGTSTP, &action,NULL);
  sigaction(SIGQUIT, &action,NULL);
  sig_fd= signalfd(-1, &action.sa_mask, SFD_CLOEXEC);
  ...
  /*無窮迴圈直到使用者輸入exit */
    ...
    wait_event:
        ...
        if (child_pid == 0) {      
            clock_gettime(CLOCK_REALTIME,&startTime); //因為使用vfork，因此child更新startTime會寫到parent的記憶體空間
