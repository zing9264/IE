...
/*
   ...
   cmd：回傳執行檔
 */
void parseString(char *str, char **pcmd) {
	int idx = 0;
	char *retPtr;
	// printf("%s\n", str);
	retPtr = strtok(str, " \n");
...
void devidevect(){
	...
	{
		if(strcmp(argVect[i],"|")!=0){
			cmd[j].argv[k]=argVect[i];
			k++;
		}
		else{
			cmd[j].argv[k]=0;
...
int main(int argc, char **argv) {
	char cmdLine[4096];
	char *exeName;
	int  wstatus,wstat;        // wstatus紀錄child的結束狀態
			...
			switch (fdsi.ssi_signo) { //判斷signal number
				case SIGINT:
					if (child_pid > 0) {
						int ret;
						if (killgrp==1) {
							fprintf(stderr,RED "\nkill process group:  " YELLOW "%d\n", -1*child_pid);
							ret=  kill(-1*child_pid, fdsi.ssi_signo);
						} else {    //殺process
							fprintf(stderr, RED"\nkill process :" YELLOW " %d\n", child_pid);
							ret =  kill(child_pid,fdsi.ssi_signo);
						}
						if (ret == -1) {
